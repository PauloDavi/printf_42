...
/*   Updated: 2023/06/23 21:22:26 by vscode           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_putnbr_base(unsigned long nbr, const char *base, t_element element)
{
	int		len;
	int		base_len;
	char	*str;
...
static void	select_prefix(char *str_n, int *min_len, t_element element)
{
	...
	if (element.type == 'x' || element.type == 'p')
		ft_strcpy(str_n, HEX_L_PREFIX, ++(*min_len));
	else if (element.type == 'o')
		ft_strcpy(str_n, "0", (*min_len));
	else if (element.type == 'X')
		ft_strcpy(str_n, HEX_U_PREFIX, ++(*min_len));
}
